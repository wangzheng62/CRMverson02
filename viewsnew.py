...
class Employeeview(View):
    def __init__(self,**kw):
        self.kw=kw
    def dispatch_request(self,temp='employeemain.html'):
        return render_template(temp, **self.kw)
rootpath='/'
urllist=[{'employeenow':{'welcome':'本功能尚未开放'}},
              {'employeesearch':{'welcome':'本功能尚未开放','employeelist':getdata()}},
              {'employeeadd':{'welcome':'本功能尚未开放'}},
              {'employeeanalyze':{'welcome':'本功能尚未开放'}}]

for d in urllist:
    for key in d:
        view=Employeeview.as_view(key,**d[key])
        app.add_url_rule(rootpath+key,view_func=view)
